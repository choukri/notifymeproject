...
public class TwilioMessageSenderClient {
    ...
    public Message sendSMS(final String to, final String body) {
        ...
        smsMessage = sendMessage(toPhoneNumber, twilioClient.getFromPhoneNumber(), body,null);

        logger.info(METHOD + smsMessage.toString());
        return smsMessage;
    }
    
    public Message sendWhatsApp(final String to, final String body, final String url) {
        final String METHOD = "sendWhatsApp ";
        Message whatsAppMessage = null;

        PhoneNumber toPhoneNumber = new PhoneNumber(TwilioConsts.WHATSAPP_PREFIX + to);
        whatsAppMessage = sendMessage(toPhoneNumber, twilioClient.getWhatsappPhoneNumber(), buildWhatsAppMessage(body),url);
    ...
    private Message sendMessage(final PhoneNumber to, final PhoneNumber from, final String body,final String url ) {
        if(url==null) {
        	return new MessageCreator(to, from, body)
            		.create(this.twilioClient.getTwilioRestClient());
        }else {
        	return new MessageCreator(to, from, body)
            		.setMediaUrl(Arrays.asList(URI.create(url)))
                    .create(this.twilioClient.getTwilioRestClient());
        }
